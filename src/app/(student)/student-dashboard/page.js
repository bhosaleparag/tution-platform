...
import { getStudentQuizzes } from '@/api/firebase/student';
...
export default function StudentDashboard() {
  ...
  const loadQuizEvents = async () => {
    try {
      setLoading(true);
      const studentEvents = await getStudentQuizzes(user.class, user.instituteId, user.uid);
      setEvents(studentEvents?.quizzes || { upcoming: [], active: [], completed: [] });
    } catch (error) {
          ...
          <div className="mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {events.active.map((event) => (
                <DashboardCard key={event.id} title={event.title || 'Quiz'}>
